...
async def start_cmd(message: Message, state: FSMContext):
 ...
 f"🆔 Telegram ID avtomatik olindi / получен автоматически: <code>{message.from_user.id}</code>",
...
async def order_phone(message: Message, state: FSMContext):
 ...
 prompt = "📍 Joylashuvingizni pastdagi tugma orqali yuboring:" if get_lang(message.from_user.id) == "uz" else "📍 Отправьте локацию кнопкой ниже:"
...
async def order_phone_wrong(message: Message):
 prompt = "Iltimos, telefon raqamingizni pastdagi tugma orqali yuboring." if get_lang(message.from_user.id) == "uz" else "Пожалуйста, отправьте номер телефона кнопкой ниже."
 await message.answer(prompt, reply_markup=contact_request_kb(message.from_user.id))
...
async def order_location_wrong(message: Message):
 prompt = "Iltimos, joylashuvingizni pastdagi tugma orqali yuboring." if get_lang(message.from_user.id) == "uz" else "Пожалуйста, отправьте локацию кнопкой ниже."
 await message.answer(prompt, reply_markup=location_request_kb(message.from_user.id))
